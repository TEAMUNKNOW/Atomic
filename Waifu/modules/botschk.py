...
@Waifu.on_message(filters.command("addbot") & filters.user(SUDO_USERS))
async def add_bot(Waifu, message):
...
@Waifu.on_message(filters.command("rmbot") & filters.user(SUDO_USERS))
async def remove_bot(Waifu, message):
    bruh = message.text.split(maxsplit=1)[1]
    data = bruh.split(" ")
