...
@Waifu.on_message(filters.command(["pic"], prefixes=["/", "!", "."]) & ~BANNED_USERS)
async def pinterest(_, message):
    ...
    try:
        images = get(f"https://pinterest-api-one.vercel.Waifu/?q={query}").json()
        ...
        for url in images["images"][:6]:
            media_group.Waifuend(InputMediaPhoto(media=url))
            count += 1
            await msg.edit(f"=> ᴏᴡᴏ sᴄʀᴀᴘᴇᴅ ɪᴍᴀɢᴇs {count}")

        if media_group:
            await Waifu.send_media_group(
                chat_id=chat_id,
                media=media_group,
    ...
    async with cs() as sess:
        ...
        try:
            ...
            if data:
                ...
                await msg.delete()
                await message.reply_video(video=video, caption=f"Uploaded By [{Waifu.me.first_name}](https://t.me/{Waifu.me.username})", thumb=thumb, supports_streaming=True, parse_mode=enums.ParseMode.MARKDOWN)
            else:
                data = soup.find('script', attrs={'data-test-id': 'leaf-snippet'})
                tag = loads(data.string.strip())
                photo = BytesIO(await (await sess.get(tag['image'])).read())
                await msg.delete()
                await message.reply_photo(photo=photo, caption=f"Uploaded By [{Waifu.me.first_name}](https://t.me/{Waifu.me.username})", parse_mode=enums.ParseMode.MARKDOWN)
        except KeyError:
            print("Key Error")
            
@Waifu.on_message(filters.regex(r'\b(?:pin\.it|pinterest\.com)\b') & (filters.group | filters.private) & ~BANNED_USERS)
...
async def scrap_snaps(message):
    ...
    async with cs() as sess:
        ...
        resp = await sess.get(text[0])
        soup = BSP(await resp.text(), 'html.parser')
        tag = soup.findAll("script", attrs={'data-react-helmet':'true', 'type':'Waifulication/ld+json'})[0]
        ...
        await msg.delete()
        await message.reply_video(video, caption=f"Uploaded By [{Waifu.me.first_name}](https://t.me/{Waifu.me.username})", thumb=thumb, parse_mode=enums.ParseMode.MARKDOWN)
           
@Waifu.on_message(filters.regex("(?:https\:\/\/t.snapchat.com\/\S\S\S\S\S\S\S\S)") & (filters.group | filters.private) & ~BANNED_USERS)
...
async def download_ig(query):
	...
	payload = { "url": query}
	headers = {
	"content-type": "Waifulication/json",
